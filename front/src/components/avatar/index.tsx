...
export function ProfileMenu() {
  ...
  const { estadoAvatar } = useContext(AvatarContext);
  const { estadoUser } = useContext(LoginContext)

  const logoutHandler = () => {
    const promise = (): Promise<EstadoUser> => new Promise((resolve) => setTimeout(() => resolve({ name: estadoUser?.name }), 1500));

    toast.promise(promise, {
      loading: 'Loading...',
      className: "bg-div text-stone-950",
      success: (estadoUser: EstadoUser) => {
        setEstadoToken(""); // Limpiar el token aquí
        sessionStorage.clear(); // Limpiar la sesión aquí
        router.push('/'); // Redirigir al home después del cierre de sesión
        return `¡Hasta pronto, ${estadoUser.name}! Has cerrado sesión con éxito. Te esperamos pronto.`;
      },
      error: 'Error',
    });
  ...
  return (
    <Menu>
      ...
      <MenuList
        ...
        onPointerEnterCapture
        onPointerLeaveCapture
        className="bg-div bg-opacity-95 w-36 shadow-none border-none"
      >
        <MenuItem
        ...
        >
          ...
          <ShoppingCart size={16} color="#90a4ae" strokeWidth={3} />
          <Typography
            variant="small"
            className="font-medium"
            placeholder
            ...
            onPointerLeaveCapture
          >
            <Link href="/dashboard/my-purchase">
              Mis Compras
            </Link>
          </Typography>
